...
class TarotService:
    ...
    def _generate_compatibility_prompt(
        ...
        return (
            ...
            "조건은 다음과 같아"
            "1. 점수를 최대한 극단적으로 줄 것. (주로 10, 20, 30, 40, 70, 80, 90 해당 점수대에 분포해줘)"
            "2. 최대한 중복없이 줄 것. (ex. 11. 21. 34, 57 같이 다양한 경우의 수를 고려해줘)"
            "3. 최대한 냉정하게 줄 것. (너무 높은 점수를 많이 주진 마.)"
            "이 정보들과 조건들을 바탕으로 두 사람의 궁합을 나타내는 문장을 다음과 같이 만들어줘. "
            "(첫 번째 사람 이름)님과 (두 번째 사람 이름)님의 궁합은 (카드의 의미를 바탕으로 해석한 두 사람 궁합의 퍼센트에이지)입니다."
            ...
            "따라서 이러한 점을 보았을 때, 두 분의 궁합은 (카드의 의미를 바탕으로 해석한 두 사람 궁합의 퍼센트에이지)입니다."
        )
